...
def load_view(user):
    ...
    for d in final_devices:
        ...
        if logs:
            ...
            # --- GÜNCELLEME: None (Boş Veri) Koruması ---
            # Veri yoksa program çökmesin diye 'or 0' veya '--' ekledik.
    ...
    if is_single_device and history_logs:
        ...
        if not df_raw.empty:
            ...
            for day, group in df_raw.groupby('date'):
                ...
                last_rec = group.iloc[-1] # GÜNÜN SON SİNYALİ
                
                safe_bat_val = int(last_rec['bat'] or 0)

                daily_stats.append({
                    "Tarih": day.strftime('%d.%m.%Y'),
                    "Veri Adedi": signal_count, # <-- YENİ SÜTUN: Kaç nokta var?
            ...
            dummy_time_str = format_duration(cumulative_hours) 
