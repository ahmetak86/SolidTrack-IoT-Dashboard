# backend/models.py (V5 - ESKİ + YENİ ÖZELLİKLER BİR ARADA)
...
# ---------------------------------------------------------
# 1. KULLANIM PROFİLLERİ (YENİLENMİŞ)
# ---------------------------------------------------------
class UtilizationProfile(Base):
    ...
    burst_mode_enabled = Column(Boolean, default=False) # Eski kodunda vardı, kalsın
    
    # [YENİ] Mod Adı (Burst, Motion Extended, Standard)
    mode_name = Column(String, default="Standard")
    
    devices = relationship("Device", back_populates="profile")

# ---------------------------------------------------------
# 2. KULLANICILAR (ESKİ SÜTUNLAR GERİ GELDİ)
# ---------------------------------------------------------
class User(Base):
    __tablename__ = 'users'
    ...
    date_format = Column(String, default='DD.MM.YYYY') # Geri geldi
    ...
    # Bildirim Ayarları (HEPSİ GERİ GELDİ)
    ...
    report_subscriptions = relationship("ReportSubscription", back_populates="user") # İlişki eklendi

# ---------------------------------------------------------
# 3. CİHAZLAR (ESKİ SÜTUNLAR GERİ GELDİ)
# ---------------------------------------------------------
class Device(Base):
    ...
    # İnce Ayarlar (Geri geldi)
    initial_hours_offset = Column(Float, default=0.0)
    min_battery_threshold = Column(Integer, default=20) # Geri geldi
    notification_email = Column(String) # Geri geldi
    
    # Limitler (Geri geldi)
    limit_shock_g = Column(Float, default=8.0) # Geri geldi
    limit_temp_c = Column(Integer, default=80) # Geri geldi
    ...
    # Hem özet hem detaylı olaylar
    utilization_logs = relationship("UtilizationLog", back_populates="device") # Eski isimle devam
    utilization_events = relationship("UtilizationEvent", back_populates="device") # Yeni tablo
...
# ESKİ TİP GÜNLÜK ÖZET (İsmini UtilizationLog olarak bıraktık ki eski kodların bozulmasın)
...
class UtilizationEvent(Base):
    ...
    is_burst = Column(Boolean, default=True)
