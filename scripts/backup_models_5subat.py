...
class Device(Base):
    ...
    # CASCADE EKLENDİ: Cihaz silinince bunlara bağlı veriler de silinsin
    ...
    alarms = relationship("AlarmEvent", back_populates="device", cascade="all, delete-orphan")
...
# ---------------------------------------------------------
# [YENİ] GELİŞMİŞ ALARM SİSTEMİ MODELLERİ (V2)
# ---------------------------------------------------------
class Alarm(Base):
    ...
    description = Column(String)
    operator = Column(String, nullable=True) # Operatör filtresi için
    
    # İlişkiler (Opsiyonel)
    # device = relationship("Device") # Gerekirse açılabilir
